...
This was done on 2016-03-03 12:10:54.600852.
*/
/**
...
void openserial_goldenImageCommands(OpenMote* self);
...
owerror_t openserial_printPacket(OpenMote* self, uint8_t* buffer, uint8_t length, uint8_t channel) {
   uint8_t  i;
   INTERRUPT_DECLARATION();
...
void openserial_stop(OpenMote* self) {
 ...
 openserial_goldenImageCommands(self);
            break;
         default:
...
void openserial_goldenImageCommands(OpenMote* self){
 ...
 openserial_getInputBuffer(self, &(input_buffer[0]),numDataBytes);
   version = (self->openserial_vars).inputBuf[1];
#ifndef GOLDEN_IMAGE_NONE
   type    = (self->openserial_vars).inputBuf[2];
#endif
...
#endif
   commandId  = (self->openserial_vars).inputBuf[3];
   commandLen = (self->openserial_vars).inputBuf[4];
   ...
   if (commandLen>3) {
       ...
       if (commandLen == 1) {
           comandParam_8 = (self->openserial_vars).inputBuf[5];
       } else {
           // commandLen == 2
           comandParam_16 = ((self->openserial_vars).inputBuf[5]      & 0x00ff) | \
                            (((self->openserial_vars).inputBuf[6]<<8) & 0xff00); 
       }
   }
 ...
 neighbors_setMyDAGrank(self, comandParam_16);
           break;
 ...
 sixtop_request(self, commandId-8,&neighbor,1);
                ...
                for (i=0;i<commandLen;i++){
                    cellList[i].tsNum           = (self->openserial_vars).inputBuf[5+i];
